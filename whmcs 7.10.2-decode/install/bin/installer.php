...
$climate->arguments->add(array("help" => array("prefix" => "h", "longPrefix" => "help", "description" => "Print usage statement", "noValue" => true), "verbose" => array("prefix" => "v", "longPrefix" => "verbose", "description" => "Print installer log to standard out", "noValue" => true), "non-interactive" => array("prefix" => "n", "longPrefix" => "non-interactive", "description" => "Non interactive. Assume Yes for confirmations", "noValue" => true), "install" => array("prefix" => "i", "longPrefix" => "install", "description" => "Perform an installation", "noValue" => true), "upgrade" => array("prefix" => "u", "longPrefix" => "upgrade", "description" => "Perform an upgrade", "noValue" => true), "status" => array("prefix" => "s", "longPrefix" => "status", "description" => "Print status information about files and database", "noValue" => true), "configuration" => array("prefix" => "c", "longPrefix" => "config", "description" => "Used in combination with non-interactive, reads configuration data from a JSON via STDIN", "noValue" => true)));
    ...
    if ($climate->arguments->defined("help")) {
        ...
        if ($climate->arguments->defined("status")) {
            ...
            if ($climate->arguments->defined("install")) {
                $config = array();
                if ($climate->arguments->defined("configuration")) {
                    if (!$climate->arguments->defined("non-interactive")) {
                        throw new Exception("Configuration argument is only valid when used with Non-Interactive");
                    }
                    ...
                    if ($config) {
                        ...
                        $config = $cli->sanitizeConfig($config);
                    }
                }
                $cmd = "install";
                $cli->header("Install WHMCS")->eula()->install($config)->footer();
            } else {
