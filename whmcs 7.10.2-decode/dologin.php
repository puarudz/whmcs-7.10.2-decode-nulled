...
if (!$loginsuccess) {
    ...
    try {
        $allowAutoAuth = WHMCS\Config\Setting::getValue(WHMCS\Authentication\Client::SETTING_ALLOW_AUTOAUTH);
        if ($allowAutoAuth && $hash) {
            ...
            if ($autoauthkey) {
                ...
                if (hash_equals($hashverify, $hash)) {
                    ...
                    if ($login_uid) {
                        ...
                        $_SESSION["upw"] = WHMCS\Authentication\Client::generateClientLoginHash($login_uid, $login_cid, $login_pwd, $email);
                        ...
                        $loginsuccess = true;
                        if ($goto) {
                            $trustedGotoUrl = (new WHMCS\Auth())->cleanRedirectUri($goto);
                            if ($trustedGotoUrl) {
