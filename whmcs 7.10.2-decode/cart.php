...
if ($a == "checkout") {
    ...
    $smartyvalues["existingCards"] = Illuminate\Support\Collection::make(array());
...
if ($a == "view") {
    ...
    if (($submit || $checkout || $validatelogin) && !$validatepromo) {
        ...
        if ($availablegateways[$paymentmethod]["type"] == WHMCS\Module\Gateway::GATEWAY_CREDIT_CARD && $ccinfo) {
            ...
            $_SESSION["cartccdetail"] = encrypt(base64_encode(serialize(array($cctype, $ccnumber, $ccexpirymonth, $ccexpiryyear, $ccstartmonth, $ccstartyear, $ccissuenum, $cccvv, $nostore, $ccinfo, $ccDescription))));
        }
        $errormessage .= cartValidationOnCheckout(WHMCS\Session::get("uid"));
    ...
    $clientPayMethods = WHMCS\User\Client::find($userid)->payMethods;
    $gatewaysoutput = $checkoutOutput = $expressCheckoutButtons = array();
...
if ($a == "complete") {
    ...
    $services = WHMCS\Service\Service::with("product")->where("userid", $userId)->where("orderid", $orderid)->where("domainstatus", WHMCS\Service\Status::PENDING);
    ...
    if (0 < $invoiceid) {
        ...
        $params = array_merge($params, $invoice->getGatewayInvoiceParams());
        if ($orderfrm->inExpressCheckout()) {
            $paramsForExpressCheckout = array_merge($params, array("orderId" => $orderid, "expressCheckout" => $orderfrm->getExpressCheckoutData()["metaData"]));
            try {
