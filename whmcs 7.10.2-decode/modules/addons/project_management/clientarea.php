...
if (!$a) {
    ...
    while ($data = mysql_fetch_array($result)) {
        ...
        $projects[] = array("id" => $data["id"], "title" => $data["title"], "adminid" => $data["adminid"], "adminname" => get_query_val("tbladmins", "CONCAT(firstname,' ',lastname)", array("id" => $data["adminid"])), "created" => fromMySQLDate($data["created"], 0, 1), "duedate" => fromMySQLDate($data["duedate"], 0, 1), "lastmodified" => fromMySQLDate($data["lastmodified"], 0, 1), "status" => $status);
    }
    ...
    if ($a == "view") {
            ...
            foreach (WHMCS\File\Upload::getUploadedFiles("attachments") as $uploadedFile) {
                try {
                    if (WHMCS\File\Upload::isExtensionAllowed($uploadedFile->getCleanName())) {
                        $file = new WHMCS\Module\Addon\ProjectManagement\Models\ProjectFile();
        ...
        $attachments = WHMCS\Module\Addon\ProjectManagement\Models\ProjectFile::where("message_id", 0)->where("project_id", $projectid)->get();
        foreach ($attachments as $attachment) {
            $filename = substr($attachment->filename, 7);
            if ($filename) {
                $attachmentsArray[$attachment->id] = array("filename" => $filename, "uploadDate" => $attachment->createdAt->toClientDateTimeFormat());
            }
        }
