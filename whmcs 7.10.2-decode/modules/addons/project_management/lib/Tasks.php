...
class Tasks extends BaseProjectEntity
{
    ...
    public function search()
    {
        ...
        $results = Models\Task\Template::where("name", "like", "%" . $search . "%")->get(array("id", "name"));
    ...
    public function deleteTaskTemplate()
    {
        ...
        return array("success" => true, "successMsgTitle" => "Deleted", "successMsg" => "Template '" . $templateName . "' Deleted");
    }
    public function saveOrder()
    {
        $taskOrder = \App::getFromRequest("task");
        foreach ($taskOrder as $order => $taskId) {
            \WHMCS\Database\Capsule::table("mod_projecttasks")->where("projectid", $this->project->id)->where("id", $taskId)->update(array("order" => $order));
        }
        return array("success" => true, "successMsgTitle" => "Updated", "successMsg" => "Task Sort Order Updated");
    }
}
