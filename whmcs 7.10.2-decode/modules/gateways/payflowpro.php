...
{
    $metaData = array("DisplayName" => "PayPal Payflow Pro", "APIVersion" => "1.1");
    $useReference = WHMCS\Database\Capsule::table("tblpaymentgateways")->where("setting", "usereftrans")->where("gateway", "payflowpro")->value("value");
    if ($useReference) {
        $metaData["TokenWorkflow"] = true;
    }
    return $metaData;
}
