...
{
    return array("FriendlyName" => array("Type" => "System", "Value" => "Authorize.net"), "loginid" => array("FriendlyName" => "Login ID", "Type" => "text", "Size" => "20"), "transkey" => array("FriendlyName" => "Transaction Key", "Type" => "password", "Size" => "20"), "testmode" => array("FriendlyName" => "Test Mode", "Type" => "yesno"));
}
function authorize_capture($params)
{
    ...
    if (!$params["testmode"] && (!isset($params["testurl"]) || !$params["testurl"])) {
        $auth->add_field("x_solution_id", "AAA172605");
    } else {
        if (isset($params["testurl"]) && $params["testurl"]) {
            $auth->add_field("x_solution_id", "AAA100302");
        }
    }
    switch ($auth->process()) {
        case 1:
        ...
        case 2:
            return array("status" => "declined", "rawdata" => $auth->dump_response(), "declinereason" => $auth->get_response_reason_text());
    }
    return array("status" => "error", "rawdata" => $auth->dump_response(), "declinereason" => $auth->get_response_reason_text());
}
