...
function worldpayinvisiblexml_3dsecure($params)
{
    ...
    WHMCS\Database\Capsule::table("tblgatewaylog")->where("gateway", "WorldPay Invisible XML Callback")->where("result", "echoData Not Found")->delete();
    WHMCS\Database\Capsule::table("tblgatewaylog")->where("gateway", "like", "%WPI%")->where("date", date("Y-m-d H:i:s", strtotime("-10 minutes")))->delete();
    WHMCS\Database\Capsule::table("tblgatewaylog")->where("gateway", "WPIORDERCODE" . $params["invoiceid"])->delete();
    WHMCS\Database\Capsule::table("tblgatewaylog")->where("gateway", "WPIECHODATA" . $params["invoiceid"])->delete();
    WHMCS\Database\Capsule::table("tblgatewaylog")->where("gateway", "WPICPDATA" . $params["invoiceid"])->delete();
    WHMCS\Database\Capsule::table("tblgatewaylog")->insert(array("date" => date("YmdHis"), "gateway" => "WPIORDERCODE" . $params["invoiceid"], "data" => $orderCode));
    WHMCS\Database\Capsule::table("tblgatewaylog")->insert(array("date" => date("YmdHis"), "gateway" => "WPIECHODATA" . $params["invoiceid"], "data" => $echoData));
    if ($params["cvvpass"]) {
        WHMCS\Database\Capsule::table("tblgatewaylog")->insert(array("date" => date("YmdHis"), "gateway" => "WPICPDATA" . $params["invoiceid"], "data" => $cvv));
    }
