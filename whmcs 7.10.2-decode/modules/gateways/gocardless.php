...
function gocardless_config(array $params)
{
    ...
    foreach ($currencies as $currencyCode) {
        if (in_array($currencyCode, WHMCS\Module\Gateway\GoCardless\GoCardless::SUPPORTED_CURRENCIES)) {
            ...
            $return["name_" . $scheme] = array("FriendlyName" => "Display Name for " . $schemeName, "Type" => "text", "Placeholder" => "Leave blank to use default");
        }
    }
    $usageNotes = array();
    foreach ($currencies as $currencyCode) {
...
function gocardless_onboarding_response_handler($params)
{
    ...
    return array("accessToken" => $accessToken, "callbackToken" => $callbackSecret);
}
...
{
    return array(array("label" => "Manage Cancelled Mandates", "actionName" => "list_cancelled_mandates", "modalSize" => "modal-lg", "modal" => true, "disabled" => empty($params["accessToken"])), array("label" => "Import Existing Mandates", "actionName" => "list_mandates_for_import", "modalSize" => "modal-lg", "modal" => true, "disabled" => empty($params["accessToken"])));
}
