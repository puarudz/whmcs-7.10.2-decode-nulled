...
function quantumvault_remoteupdate($params)
{
    ...
    $quantum = quantumvault_getCode($params["apiusername"], $params["apikey"], "550", "420", "0", "0", $params["gatewayid"], "CustomerEdit");
...
function quantumvault_getCode($API_Username, $API_Key, $width, $height, $amount = "0", $id = "0", $custid = "0", $method = "0", $addtoVault = "N", $skipshipping = "N")
{
    ...
    if (is_array($response) && $response[1] != "error") {
        ...
        $content = "<iframe src=\"https://secure.quantumgateway.com/cgi/ilf.php?k=" . $response[1] . "&ip=" . $_SERVER["REMOTE_ADDR"] . $extrapars . "\" height=\"" . $height . "\" width=\"" . $width . "\" frameborder=\"0\"></iframe>";
        $wontReflectChangesText = defined("ADMINAREA") ? AdminLang::trans("payments.creditCardChangesWontBeReflected") : Lang::trans("paymentMethods.creditCardChangesWontBeReflected");
        $content .= "<div class=\"alert alert-success\">" . $wontReflectChangesText . "</div>";
        $thereturn["iframe"] = $content;
