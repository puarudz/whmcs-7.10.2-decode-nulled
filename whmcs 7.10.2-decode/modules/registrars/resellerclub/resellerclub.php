...
function resellerclub_config_validate(array $params)
{
    if (!$params["ResellerID"]) {
        throw new WHMCS\Exception\Module\InvalidConfiguration("Missing Reseller ID. Please navigate to Setup > Domain Registrars to configure.");
    }
    if (!$params["APIKey"]) {
        throw new WHMCS\Exception\Module\InvalidConfiguration("Missing API Key. Please navigate to Setup > Domain Registrars to configure.");
    }
    ...
    foreach ($typelist as $type) {
        ...
        if (0 < $numResultRecords) {
            foreach ($result as $entry => $value) {
                ...
                if ($host && $address) {
                    $hostrecords[] = array("hostname" => $host, "type" => (string) $type, "address" => $address, "priority" => (string) $priority, "recid" => $recid);
                }
            }
        }
    ...
    if ($errorNumber) {
        ...
        switch ($errorNumber) {
            ...
            case CURLE_SSL_CONNECT_ERROR:
                $href = "https://docs.whmcs.com/ResellerClub#IP_Registration";
                $errorMessage = "You must authorize your IP address to use this API. " . (string) $ipMessage . ". " . "<a href=\"" . $href . "\" class=\"autoLinked\">" . "Learn More</a>";
                break;
            default:
    ...
    if ($params["domainObj"]->getLastTLDSegment() == "uk") {
        ...
        if ($params["domainObj"]->getLastTLDSegment() == "eu") {
            ...
            if ($params["domainObj"]->getLastTLDSegment() == "cn") {
                ...
                if ($params["domainObj"]->getLastTLDSegment() == "cl") {
...
function resellerclub_get_reseller_details(array $params)
{
    ...
    $postFields["auth-userid"] = $params["ResellerID"];
    $postFields["api-key"] = $params["APIKey"];
    $resellerDetails = resellerclub_sendcommand("details", "resellers", $postFields, array(), "GET", true);
    if (is_array($resellerDetails)) {
        ...
        if (array_key_exists("status", $check) && strtolower($check["status"]) == "error") {
            return array("error" => $check["message"]);
        }
    }
    return $resellerDetails;
}
function resellerclub_get_pricing_data(array $params)
{
    ...
    $postFields["auth-userid"] = $params["ResellerID"];
    $postFields["api-key"] = $params["APIKey"];
    $pricingData = resellerclub_sendcommand("reseller-cost-price", "products", $postFields, array(), "GET", true);
    return $pricingData;
}
function resellerclub_get_category_data(array $params)
{
    ...
    $postFields["auth-userid"] = $params["ResellerID"];
    $postFields["api-key"] = $params["APIKey"];
    $categoryData = resellerclub_sendcommand("details", "products", $postFields, array(), "GET", true);
    return $categoryData;
}
function resellerclub_get_tld_to_category_map(array $params)
{
    ...
    $postFields["auth-userid"] = $params["ResellerID"];
    $postFields["api-key"] = $params["APIKey"];
    $data = resellerclub_sendcommand("category-keys-mapping", "products", $postFields, array(), "GET", true);
    $data = $data["domorder"];
    $tldMap = array();
...
function resellerclub_get_currencies_data(array $params)
{
    ...
    $postFields["auth-userid"] = $params["ResellerID"];
    $postFields["api-key"] = $params["APIKey"];
    return resellerclub_sendcommand("details", "currency", $postFields, array(), "GET", true);
}
