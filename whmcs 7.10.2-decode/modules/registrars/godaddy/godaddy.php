...
function godaddy_TransferDomain(array $params)
{
    ...
    try {
        $response = $goDaddyClient->post("domains/" . $domain . "/transfer", array("headers" => array("X-Shopper-Id" => $shopperId), "json" => $params));
        $response = json_decode($response, true);
