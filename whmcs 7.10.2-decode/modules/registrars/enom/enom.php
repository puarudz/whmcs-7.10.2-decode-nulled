...
function enom_config_validate(array $params)
{
    ...
    $eNom->AddParam("command", "SetCustomerDefinedData");
    $eNom->AddParam("ObjectID", 1);
    $eNom->AddParam("Type", 1);
    $eNom->AddParam("Key", "WHMCS");
    $eNom->AddParam("Value", $params["Username"]);
    $eNom->AddParam("DisplayFlag", 0);
    $eNom->AddParam("EnteredBy", "WHMCS");
    $eNom->DoTransaction($params);
    if (0 < $eNom->Values["ErrCount"]) {
        $error = $eNom->Values["Err1"];
        if (substr($error, 0, 39) == "User not permitted from this IP address") {
            $ip = WHMCS\Environment\WebServer::getExternalCommunicationIp();
            $href = "https://docs.whmcs.com/Enom#IP_Registration";
            $error = "You must authorize your IP address to use this API. Your IP is: " . $ip . ". <a href=\"" . $href . "\" class=\"autoLinked\">Learn More</a>";
...
function enom_GetTldPricing(array $params)
{
    ...
    $eNom->AddParam("pw", $params["Password"]);
    $eNom->AddParam("ResponseType", "XML");
    $eNom->AddParam("command", "PE_GetDomainPricing");
    $eNom->DoTransaction($params, false);
    $arrayData = $eNom->parseXMLResponseToArray("tag");
    $errorMessages = array();
    $errorCount = $arrayData["interface-response"]["ErrCount"];
    for ($i = 1; $i <= $errorCount; $i++) {
        $errorMessages[] = trim($arrayData["interface-response"]["errors"]["Err" . $i]);
    }
    if (0 < count($errorMessages)) {
        return array("error" => implode(", ", $errorMessages));
    }
    $tldPricing = $arrayData["interface-response"]["pricestructure"]["product"];
    $results = new WHMCS\Results\ResultsList();
    foreach ($tldPricing as $tldPrice) {
        $item = (new WHMCS\Domain\TopLevel\ImportItem())->setExtension($tldPrice["tld"])->setCurrency("USD")->setRegisterPrice($tldPrice["resellerpricereg"])->setMaxYears(10)->setYearsStep(1);
        if ($tldPrice["renewenabled"] === "True") {
            $item->setRenewPrice($tldPrice["resellerpricerenew"]);
        }
        if ($tldPrice["transferenabled"] === "True") {
            $item->setTransferPrice($tldPrice["resellerpricetran"]);
        }
        if ($tldPrice["rgpenabled"] === "True") {
            $item->setRedemptionFeePrice($tldPrice["resellerpricergp"]);
        }
        if ($item->getExtension() == ".tm") {
            $item->setMinYears(10)->setRegisterPrice($tldPrice["resellerpricereg"] * 10)->setRenewPrice($tldPrice["resellerpricerenew"] * 10)->setTransferPrice($tldPrice["resellerpricetran"] * 10);
        } else {
            if ($item->getExtension() == ".com.mx") {
                $item->setMaxYears(5);
            }
        }
        $results[] = $item;
                                                                ...
                                                                if ($params["domainObj"]->getLastTLDSegment() == "fr") {
                                                                    ...
                                                                    if ($params["domainObj"]->getLastTLDSegment() == "nu") {
                                                                        ...
                                                                        if ($params["domainObj"]->getLastTLDSegment() == "quebec") {
                                                                            ...
                                                                            if ($params["domainObj"]->getLastTLDSegment() == "dev") {
                                                                                $devAgreement = "";
                                                                                if ($params["additionalfields"][".DEV SSL Agreement"]) {
                                                                                    $devAgreement = "I Agree";
                                                                                }
                                                                                $Enom->AddParam("dev_agree", $devAgreement);
                                                                            }
                                                                        }
                                                                    }
                                                                }
