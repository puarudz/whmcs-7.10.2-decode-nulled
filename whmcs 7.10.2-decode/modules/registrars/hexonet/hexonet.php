...
function hexonet_CheckAvailability($params)
{
    ...
    foreach ($hexFlattenResults as $domain => $attributes) {
        ...
        if (isset($params["suggestions"])) {
            if ($domainSearchResult->getStatus() !== WHMCS\Domains\DomainLookup\SearchResult::STATUS_REGISTERED) {
...
function hexonet_TransferDomain($params)
{
    ...
    $command = array("COMMAND" => "TransferDomain", "DOMAIN" => $domain, "PERIOD" => $params["regperiod"], "NAMESERVER0" => $params["ns1"], "NAMESERVER1" => $params["ns2"], "NAMESERVER2" => $params["ns3"], "NAMESERVER3" => $params["ns4"], "OWNERCONTACT0" => $registrant, "ADMINCONTACT0" => $admin, "TECHCONTACT0" => $admin, "BILLINGCONTACT0" => $admin, "AUTH" => $eppcode);
