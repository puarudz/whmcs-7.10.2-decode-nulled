...
function cpanel_request_backup(array $params)
{
    ...
    switch ($params["dest"]) {
        case "passiveftp":
            $postVarsData = array("variant" => "passive", "username" => $params["user"], "password" => $params["pass"], "host" => $params["hostname"], "port" => $params["port"], "directory" => $params["rdir"], "email" => $params["email"]);
            $dest = "_to_ftp";
            break;
        case "scp":
            $postVarsData = array("username" => $params["user"], "password" => $params["pass"], "host" => $params["hostname"], "port" => $params["port"], "directory" => $params["rdir"], "email" => $params["email"]);
            $dest = "_to_scp_with_password";
            break;
        case "homedir":
            $postVarsData = array("email" => $params["email"]);
            $dest = "_to_homedir";
            break;
        default:
            $postVarsData = array("username" => $params["user"], "password" => $params["pass"], "host" => $params["hostname"], "port" => $params["port"], "directory" => $params["rdir"], "email" => $params["email"]);
    ...
    $postVarsConnData = array("cpanel_jsonapi_user" => strtolower($params["username"]), "cpanel_jsonapi_apiversion" => "3", "cpanel_jsonapi_module" => "Backup", "cpanel_jsonapi_func" => "fullbackup" . $dest);
    ...
    try {
        ...
        if ($response["status"] == 1) {
            foreach ($response["acct"] as $userAccount) {
                ...
                $account = array("name" => $userAccount["user"], "email" => $userAccount["email"], "username" => $userAccount["user"], "domain" => $userAccount["domain"], "uniqueIdentifier" => $userAccount["domain"], "product" => $plan, "primaryip" => $userAccount["ip"], "created" => $createdDate, "status" => $status);
                $accounts[] = $account;
            }
...
{
    $command = "/json-api/listaccts";
    $postVars = array("searchtype" => "user", "search" => $params["username"], "want" => "domain,user,plan,ip,suspended,email,owner");
    $accountData = array();
    try {
        $results = cpanel_jsonrequest($params, $command, $postVars);
        if ($results["status"] == 1) {
            $userData = $results["acct"][0];
            $accountData = array("name" => $userData["user"], "email" => $userData["email"], "username" => $userData["user"], "domain" => $userData["domain"], "uniqueIdentifier" => $userData["domain"], "product" => $userData["plan"]);
            return array("success" => true, "userData" => $accountData);
        }
        return array("success" => false, "userData" => $accountData, "error" => $results["metadata"]["reason"]);
    } catch (Exception $e) {
        return array("success" => false, "userData" => $accountData, "error" => $e->getMessage());
    }
}
    ...
    $serverUsage = function (WHMCS\UsageBilling\Contracts\Metrics\ProviderInterface $provider, $tenant = NULL) use($params) {
        ...
        foreach ($results as $username => $data) {
            ...
            if (!empty($data) && $isReseller) {
                ...
                $data[] = array("id" => "subaccounts", "_count" => $subAccounts);
            }
            foreach ($data as $stat) {
                ...
                if (isset($metrics[$name])) {
                    ...
                    if (in_array($stat["units"], array("MB", "GB", "KB", "B"))) {
                        $units = $metric->units();
                        $to = $units->suffix();
...
function cpanel_GetStatsUAPI(array $params)
{
    ...
    $apiData = array("api.version" => "1", "cpanel.module" => "StatsBar", "cpanel.function" => "get_stats", "cpanel.user" => strtolower($usernames[0]));
    ...
    $commands = array();
    foreach ($usernames as $username) {
        $apiData = array("cpanel.module" => "StatsBar", "cpanel.function" => "get_stats", "cpanel.user" => strtolower($username), "display" => "addondomains|bandwidthusage|diskusage|emailaccounts|" . "mysqldatabases|mysqldiskusage|parkeddomains|postgresqldatabases|" . "postgresdiskusage|subdomains");
...
{
    $command = "/json-api/cpanel";
    $postVars = array("display" => "addondomains|bandwidthusage|diskusage|emailaccounts|" . "mysqldatabases|mysqldiskusage|parkeddomains|postgresqldatabases|" . "postgresdiskusage|subdomains", "cpanel_jsonapi_user" => strtolower($params["username"]), "cpanel_jsonapi_apiversion" => "3", "cpanel_jsonapi_module" => "StatsBar", "cpanel_jsonapi_func" => "get_stats");
    $response = cpanel_jsonrequest($params, $command, $postVars);
    if (!empty($response["result"]["error"])) {
        $error = is_array($response["result"]["error"]) ? implode(". ", $response["result"]["error"]) : $response["result"]["error"];
        throw new WHMCS\Exception("Unable to get stats: " . $error);
    }
    $data = !empty($response["result"]["data"]) && is_array($response["result"]["data"]) ? $response["result"]["data"] : array();
    return $data;
}
function cpanel_ListResellers(array $params)
{
    $command = "/json-api/listresellers";
    $postVars = array("user" => $params["username"], "api.version" => 1);
    try {
        $response = cpanel_jsonrequest($params, $command, $postVars);
        if (!is_array($response)) {
            if (!empty($response)) {
                return array("success" => false, "error" => $response, "data" => array());
            }
            return array("success" => false, "error" => "An unknown error occurred", "data" => array());
        }
        ...
        if ($resultCode == 0 || !isset($response["data"]["reseller"])) {
            if (isset($metadata["reason"])) {
                return array("success" => false, "error" => $metadata["reason"], "data" => array());
            }
            return array("success" => false, "error" => "An unknown error occurred", "data" => array());
        }
        return array("success" => true, "error" => "", "data" => $response["data"]["reseller"]);
    } catch (Exception $e) {
        return array("success" => false, "error" => $e->getMessage(), "data" => array());
    }
}
function cpanel_ResellerStats(array $params)
{
    ...
    $postVars = array("api.version" => "1", "user" => $reseller);
    $stats = array();
    ...
    if (is_array($output) && isset($output["data"]["reseller"]) && is_array($output["data"]["reseller"])) {
        ...
        $stats = array("diskusage" => $diskUsed, "disklimit" => $diskLimit, "bwusage" => $bwUsed, "bwlimit" => $bwLimit, "accounts" => $accounts, "isOwner" => $isOwner, "lastupdate" => WHMCS\Carbon::now()->toDateTimeString());
    }
    return $stats;
}
