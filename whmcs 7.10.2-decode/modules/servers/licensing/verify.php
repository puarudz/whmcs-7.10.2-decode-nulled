...
if ($addonId) {
    ...
    $licensing_secretkey = $model->productAddon->moduleConfiguration()->where("setting_name", "=", "configoption8")->first()->value;
...
function licensing_getlicreturndata($licenseid)
{
    ...
    if ($model instanceof WHMCS\Service\Service) {
        ...
        foreach ($model->addons as $addon) {
            ...
            $addons .= "name=" . $name . ";nextduedate=" . $addon->nextDueDate . ";status=" . $addon->status . "|";
        }
