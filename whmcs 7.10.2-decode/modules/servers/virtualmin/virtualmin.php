...
function virtualmin_UsageUpdate($params)
{
    ...
    foreach ($dataArray as $values) {
        ...
        $getScalarValue = function ($key, array $data) {
        ...
        $diskusage = $getScalarValue("server_byte_quota_used", $domainData) / 1048576;
        $disklimit = $getScalarValue("server_block_quota", $domainData) / 1024;
        $bwlimit = $getScalarValue("bandwidth_byte_limit", $domainData) / 1048576;
        $bwused = $getScalarValue("bandwidth_byte_usage", $domainData) / 1048576;
