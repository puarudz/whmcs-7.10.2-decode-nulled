...
function directadmin_req($command, $fields, $params, $post = false)
{
    ...
    $resultsarray = array();
    if (!$user || !$pass) {
        $resultsarray = array("error" => "1", "details" => "Login Details Missing");
        return $resultsarray;
    }
    $fieldstring = "";
        ...
        if (strpos($data, "DirectAdmin Login") == true || strpos($data, "AUTH_METHOD: \"CMD_LOGIN\"") == true) {
            ...
            if (strpos($data, "Your IP is blacklisted") !== false) {
                ...
                if (strtolower(trim($data)) === "use https") {
                    $resultsarray = array("error" => "1", "details" => "Secure Connection Is Required");
                } else {
                    if ($params["getip"]) {
    ...
    foreach ($commands as $command) {
        ...
        if (array_key_exists("list", $response)) {
            foreach ($response["list"] as $user) {
                ...
                $account = array("name" => $response2["username"], "email" => $response2["email"], "username" => $response2["username"], "domain" => $response2["domain"], "uniqueIdentifier" => $response2["domain"], "product" => $response2["package"], "primaryip" => $response2["ip"], "created" => $createdDate, "status" => $status);
                $accounts[] = $account;
            }
        }
    }
...
function directadmin_GetStats(array $params)
{
    ...
    $postVars = array("user" => strtolower($username));
