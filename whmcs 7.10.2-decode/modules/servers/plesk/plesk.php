...
{
    return array("DisplayName" => "Plesk", "APIVersion" => "1.1", "ListAccountsUniqueIdentifierDisplayName" => "Domain", "ListAccountsUniqueIdentifierField" => "domain", "ListAccountsProductField" => "configoption1", "PasswordGenerationSpecialCharacters" => "!@#\$%^&*?_~");
}
...
{
    $data = http_build_query(array("userid" => $params["userid"], "id" => $params["serviceid"], "modop" => "custom", "ac" => "DetermineUserState", "token" => generate_token("plain")));
    ...
    try {
        ...
        if (!isset($accountInfo["login"])) {
            ...
            if ($accountInfo["login"] == $params["username"]) {
                ...
                $responseString = $translator->translate("FIELD_CHANGE_PASSWORD_ADDITIONAL_PACKAGE_DESCR", array("PACKAGE" => $params["domain"]));
            }
        }
    } catch (Exception $e) {
        $responseString = Plesk_Registry::getInstance()->translator->translate("ERROR_COMMON_MESSAGE", array("CODE" => $e->getCode(), "MESSAGE" => $e->getMessage()));
    }
    $response = new WHMCS\Admin("View Clients Products/Services");
    $response->jsonResponse(array("string" => $responseString));
}
