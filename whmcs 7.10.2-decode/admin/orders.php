        ...
        foreach (WHMCS\Service\Upgrade\Upgrade::where("orderid", $id)->get() as $upgrade) {
            if ($upgrade->type == "package") {
                ...
                if ($upgrade->type == "configoptions") {
                    ...
                    $result2 = select_query("tblhosting", "tblproducts.name AS productname,domain,tblhosting.userid", array("tblhosting.id" => $upgrade->relid), "", "", "", "tblproducts ON tblproducts.id=tblhosting.packageid");
                    $data = mysql_fetch_array($result2);
