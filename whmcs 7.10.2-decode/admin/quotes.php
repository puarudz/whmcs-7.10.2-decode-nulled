    ...
    if ($action == "loadprod") {
        ...
        if ($action == "addproduct") {
            ...
            if ($billingcycle == "onetime") {
                ...
                if (!isset($pricing["cycles"][$billingcycle])) {
                    list($billingcycle) = array_keys($pricing["cycles"]);
                }
            }
        ...
        if (!$action) {
            ...
            while ($data = mysql_fetch_array($result)) {
                ...
                $stage = $data["stage"];
                if (!empty($stage)) {
                    $stage = AdminLang::trans("quotes.stage" . str_replace(" ", "", strtolower($stage)));
                }
                $total = $data["total"];
            ...
            echo $aInt->sortableTable(array(array("id", AdminLang::trans("fields.id")), array("subject", AdminLang::trans("quotes.subject")), AdminLang::trans("fields.clientname"), array("stage", AdminLang::trans("quotes.stage")), array("total", AdminLang::trans("quotes.total")), array("validuntil", AdminLang::trans("quotes.validuntil")), array("lastmodified", AdminLang::trans("fields.lastmodified")), "", ""), $tabledata, $tableformurl, $tableformbuttons);
        } else {
            if ($action == "manage") {
                if ($id) {
