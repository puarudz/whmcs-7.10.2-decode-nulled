...
$relatedService = "";
$relatedServiceType = "";
if (in_array($action, array("viewticket", "view", "gettags", "addTag", "removeTag", "split", "getmsg", "getticketlog", "getclientlog", "gettickets", "updatereply", "makingreply", "endreply", "checkstatus", "changestatus", "changeflag", "loadpredefinedreplies", "getpredefinedreply", "getquotedtext", "getcontacts", "markdown", "parseMarkdown"))) {
            ...
            if ($action == "blockdeletetickets") {
                ...
                if ($action == "closetickets") {
                    ...
                    if (!$action) {
                        ...
                        echo AdminLang::trans("global.any");
                        echo "\">\n                ";
                        $selectableOptions = array_merge(array("any"), WHMCS\Database\Capsule::table("tbltickettags")->groupBy("tag")->orderBy("tag", "asc")->pluck("tag"));
                        $selectedOptions = array_merge(array($tag), $multiTags);
                        ...
                        if ($aInt->hasPermission("View Flagged Tickets")) {
                            ...
                            foreach ($adminUsers as $adminUser) {
                                ...
                                echo "<option value=\"" . $adminUser->id . "\"" . $selected . ">" . (string) $adminUser->fullName . "</option>";
                            }
                            echo "                </select>\n            </td>\n        </tr>\n    ";
                        }
                        ...
                        if (!$tag && !$searchFlag && (!$view || in_array($view, $activeAwaitingTicketStatuses))) {
                            ...
                            $criteria = array("status" => $view, "multiStatus" => $multiView, "deptid" => $deptid, "multiDeptIds" => $multiDeptIds, "priority" => $priority, "subject" => $subject, "tag" => $tag, "multiTags" => $multiTags, "client" => $client, "clientid" => $clientid, "clientname" => $clientname, "email" => $email, "flag" => WHMCS\Auth::getID());
                        ...
                        $tbl->setColumns(array("checkall", "", array("deptname", $aInt->lang("support", "department")), array("title", $aInt->lang("fields", "subject")), $aInt->lang("support", "submitter"), array("status", $aInt->lang("fields", "status")), array("lastreply", $aInt->lang("support", "lastreply"))));
                        $ticketsModel = new WHMCS\Tickets($pageObj);
                        $criteria = array("status" => $view, "multiStatus" => $multiView, "deptid" => $deptid, "multiDeptIds" => $multiDeptIds, "priority" => $priority, "subject" => $subject, "tag" => $tag, "multiTags" => $multiTags, "client" => $client, "clientid" => $clientid, "clientname" => $clientname, "email" => $email, "notflaggedto" => $notFlaggedTo, "searchFlag" => $searchFlag, "tag_ticket_ids" => $taggedTickets);
                        $ticketsModel->execute($criteria);
                        $ticketslist = $pageObj->getData();
                        foreach ($ticketslist as $ticket) {
                            $tbl->addRow(array("<input type=\"checkbox\" name=\"selectedtickets[]\" value=\"" . $ticket["id"] . "\" class=\"checkall\" />", "<img src=\"images/" . strtolower($ticket["priority"]) . "priority.gif\" width=\"16\" height=\"16\" alt=\"" . $ticket["priority"] . "\" class=\"absmiddle\" />", $ticket["department"], "<a href=\"supporttickets.php?action=view&id=" . $ticket["id"] . "\"" . ($ticket["unread"] ? " style=\"font-weight:bold;\"" : "") . " title=\"" . $ticket["textsummary"] . "\">#" . $ticket["ticketnum"] . " - " . $ticket["subject"] . "</a>", $ticket["clientname"], $ticket["status"], $ticket["lastreply"]));
                    ...
                    if ($action == "viewticket" || $action == "view") {
                        ...
                        if ($action == "open") {
                            ...
                            echo AdminLang::trans("clientsummary.contacts");
                            echo "</td>\n        <td class=\"fieldarea\" id=\"contacthtml\" colspan=\"3\">";
                            echo $contactsdata;
                            echo "</td>\n    </tr>\n    <tr>\n        <td class=\"fieldlabel\">\n            ";
                            ...
                            echo AdminLang::trans("transactions.commaseparated");
                            echo ")\n        </td>\n    </tr>\n    <tr>\n        <td class=\"fieldlabel\">\n            ";
                            ...
                            echo "                </td>\n            </tr>\n            <tr data-original=\"true\" class=\"hidden\">\n                <td>\n                    <label>\n                        <input type=\"radio\"\n                               name=\"related_service[]\"\n                               data-type=\"\"\n                               value=\"\"\n                               checked=\"checked\"\n                        >\n                    </label>\n                </td>\n                <td colspan=\"6\">";
