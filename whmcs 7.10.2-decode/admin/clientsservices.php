...
if ($action == "refreshStats") {
    ...
    if ($tenant && $server) {
        try {
            ...
            $aInt->jsonResponse(array("success" => true, "body" => $table));
        } catch (Exception $e) {
    ...
    $aInt->jsonResponse(array("success" => false, "error" => $errorMsg));
}
ob_start();
$adminbuttonarray = "";
...
if ($modop == "terminate") {
    ...
    $result = ModuleCallFunction("Terminate", $id, array("keepZone" => $keepZone, "invoiceUsage" => $invoiceUsage), $aid);
    WHMCS\Cookie::set("ModCmdResult", $result);
    $extra = "";
    if ((int) $aid) {
        $extra = "&aid=" . $aid;
...
foreach ($metrics as $serviceMetric) {
    ...
    if ($serviceMetric->systemName() == "diskusage") {
        ...
        if ($serviceMetric->systemName() == "bandwidthusage") {
...
if ($aid) {
    ...
    if ($subscriptionid) {
        ...
        if ($gateway->functionExists("get_subscription_info")) {
            $route = routePathWithQuery("admin-addons-subscription-info", array($id), array("token" => generate_token("plain")));
        ...
        if (0 < count($manageSubButtons)) {
            ...
            $cancelSubscription = "<span class=\"input-group-btn\" style=\"display:block;\">" . $buttons . "</span>";
        }
    }
    ...
    $tbl->add($aInt->lang("fields", "nextduedate"), $aid && is_numeric($aid) && in_array($billingcycle, array("One Time", "Free Account")) ? AdminLang::trans("global.na") : $frm->date("nextduedate", $nextduedate, 12, false, "form-control date-picker-single future"));
    ...
    $tbl->add(AdminLang::trans("fields.subscriptionid"), "<div class=\"" . ($cancelSubscription ? "input-group" : "input-300") . "\" id=\"subscription\">" . $frm->text("subscriptionid", $subscriptionid, "25", false, "form-control") . $cancelSubscription . "</div>" . "<div id='subscriptionworking' style='display:none;text-align:center;'><img src='images/loader.gif' />" . "&nbsp; " . $aInt->lang("global", "working") . "</div>", true);
    ...
    if ($subscriptionid) {
        ...
        $manageSubButtons = array();
        if ($gateway->functionExists("get_subscription_info")) {
            $route = routePathWithQuery("admin-services-subscription-info", array($id), array("token" => generate_token("plain")));
            $title = AdminLang::trans("subscription.info");
            $manageSubButtons[] = "<a href=\"" . $route . "\" class=\"btn btn-default open-modal\" data-modal-title=\"" . $title . "\">" . AdminLang::trans("global.getSubscriptionInfo") . "</a>";
        }
        if ($gateway->functionExists("cancelSubscription")) {
    ...
    $tbl->add($aInt->lang("fields", "nextduedate"), in_array($billingcycle, array("One Time", "Free Account")) ? AdminLang::trans("global.na") : $frm->hidden("oldnextduedate", $nextduedate) . $frm->date("nextduedate", $nextduedate, 12, false, "form-control date-picker-single future"));
    ...
    $tbl->add($aInt->lang("fields", "promocode"), $frm->dropdown("promoid", $promoarr, $promoid, "", "", true) . "<br />(" . $aInt->lang("services", "noaffect") . ")" . $recurCountString);
    ...
    $tbl->add($aInt->lang("fields", "subscriptionid"), "<div class=\"" . ($cancelSubscription ? "input-group" : "input-300") . "\" id=\"subscription\">" . $frm->text("subscriptionid", $subscriptionid, "25", false, "form-control") . $cancelSubscription . "</div>" . "<div id='subscriptionworking' style='display:none;text-align:center;'><img src='images/loader.gif' />" . "&nbsp; " . $aInt->lang("global", "working") . "</div>", true);
...
if ($whmcs->get_req_var("ajaxupdate")) {
    ...
    $aInt->jsonResponse(array("body" => $jQueryCodeContent . $content));
} else {
    ...
    if ($moduleInterface) {
        ...
        if ($serviceUsage->hasUsageForInvoicing()) {
            $labelText = AdminLang::trans("services.invoiceUsage") . " " . "(<a href='https://docs.whmcs.com/Usage_Billing#Invoicing_and_Billing' class='autoLinked'>" . AdminLang::trans("global.learnMore") . "</a>)";
