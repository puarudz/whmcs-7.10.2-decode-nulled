...
if ($markunpaid) {
    check_token("WHMCS.admin.default");
    foreach ($selectedinvoices as $invid) {
        WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invid)->update(array("status" => WHMCS\Billing\Invoice::STATUS_UNPAID, "datepaid" => "0000-00-00 00:00:00", "date_cancelled" => "0000-00-00 00:00:00", "date_refunded" => "0000-00-00 00:00:00"));
...
if ($markcancelled) {
    check_token("WHMCS.admin.default");
    foreach ($selectedinvoices as $invid) {
        WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invid)->update(array("status" => WHMCS\Billing\Invoice::STATUS_CANCELLED, "date_cancelled" => WHMCS\Carbon::now()->toDateTimeString()));
...
$dateFilters = array("invoicedate" => "date", "duedate" => "duedate", "datepaid" => "datepaid", "last_capture_attempt" => "last_capture_attempt", "date_refunded" => "date_refunded", "date_cancelled" => "date_cancelled");
