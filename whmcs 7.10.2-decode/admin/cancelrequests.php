...
if ($action == "delete") {
    ...
    if ($action == "detail") {
        ...
        $cancellation = WHMCS\Database\Capsule::table("tblcancelrequests")->where("tblcancelrequests.id", $id)->first();
        $service = WHMCS\Service\Service::with("order")->find($cancellation->relid);
        $order = $service->order;
        $currency = getCurrency($service->userid);
        $nextDue = WHMCS\Carbon::safeCreateFromMySqlDate($service->nextDueDate);
        $nextDueOutput = "";
        if ($nextDue && $nextDue instanceof WHMCS\Carbon) {
            $nextDueOutput = $nextDue->toAdminDateFormat();
        }
        $data = array(array(AdminLang::trans("fields.ordernum") => $order ? $order->orderNumber : "", AdminLang::trans("fields.regdate") => $service ? $service->registrationDate->toAdminDateFormat() : ""), array(AdminLang::trans("fields.recurringamount") => formatCurrency($service->amount), AdminLang::trans("fields.billingcycle") => $service->billingCycle), array(AdminLang::trans("fields.nextduedate") => $nextDueOutput, AdminLang::trans("fields.status") => $service->domainStatus));
        $aInt->jsonResponse(array("output" => WHMCS\View\Bootstrap::renderKeyValuePairsInColumns(4, $data)));
    }
}
...
foreach ($result as $data) {
    ...
    $expandIcon = "<a href=\"cancelrequests.php?action=detail&id=" . $id2 . "\" class=\"view-detail\"><i class=\"fa fa-plus\"></i></a>";
    $tabledata[] = array($date, $xname, "<textarea rows=\"3\" class=\"form-control\" readonly>" . $reason . "</textarea>", $type, $cancellationDate, $expandIcon, "<a href=\"#\" onClick=\"doDelete('" . $id2 . "');return false\"><img src=\"images/delete.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"" . $aInt->lang("global", "delete") . "\"></a>");
}
