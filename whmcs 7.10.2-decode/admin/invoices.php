...
if ($action == "") {
    ...
    if ($action == "edit") {
        ...
        if ($saveoptions) {
            check_token("WHMCS.admin.default");
            $invoicedate = App::getFromRequest("invoicedate");
            $datedue = App::getFromRequest("datedue");
            $updateArray = array("date" => toMySQLDate($invoicedate), "duedate" => toMySQLDate($datedue), "paymentmethod" => $paymentmethod, "invoicenum" => $invoicenum, "taxrate" => $taxrate, "taxrate2" => $taxrate2, "status" => $status);
            if ($oldpaymentmethod != $paymentmethod) {
                $updateArray["paymethodid"] = "NULL";
            }
            update_query("tblinvoices", $updateArray, array("id" => $id));
        ...
        if ($sub == "statuscancelled") {
            check_token("WHMCS.admin.default");
            $set = array("status" => WHMCS\Billing\Invoice::STATUS_CANCELLED, "datepaid" => "0000-00-00 00:00:00", "date_cancelled" => WHMCS\Carbon::now()->toDateTimeString());
            WHMCS\Database\Capsule::table("tblinvoices")->where("id", $id)->update($set);
        ...
        if ($sub == "statusunpaid") {
            ...
            $set = array("status" => WHMCS\Billing\Invoice::STATUS_UNPAID, "datepaid" => "0000-00-00 00:00:00", "date_cancelled" => "0000-00-00 00:00:00", "date_refunded" => "0000-00-00 00:00:00");
            WHMCS\Database\Capsule::table("tblinvoices")->where("id", $id)->update($set);
        ...
        if ($removecredit != "0.00" && $removecredit != "") {
            ...
            if ($credit < $removecredit) {
                ...
                if ($status == "Paid") {
                    WHMCS\Database\Capsule::table("tblinvoices")->where("id", $id)->update(array("status" => WHMCS\Billing\Invoice::STATUS_REFUNDED, "date_refunded" => WHMCS\Carbon::now()->toDateTimeString()));
                }
        ...
        if (in_array($status, array("Paid", "Cancelled", "Draft")) || !function_exists($paymentmethod . "_capture") || $paymentmethod === "offlinecc") {
