...
function calcCartTotals($checkout = false, $ignorenoconfig = false, array $currency = array())
{
        ...
        foreach ($cartProducts as $key => $productdata) {
            ...
            if ($checkout) {
                ...
                for ($qtycount = 1; $qtycount <= $qty; $qtycount++) {
                    ...
                    if ($qtycount === 1) {
                        ...
                        $adminemailitems .= $_LANG["firstpaymentamount"] . ": " . (string) $firstPayAmount . "<br>\n";
                        $recurAmount = new WHMCS\View\Formatter\Price($product_recurring_db, $currency);
                        if ($product_recurring_db) {
                            $adminemailitems .= $_LANG["recurringamount"] . ": " . (string) $recurAmount . "<br>\n";
                        }
                        $paymentTerm = str_replace(array("-", " "), "", strtolower($databasecycle));
                        $adminemailitems .= $_LANG["orderbillingcycle"] . ": " . $_LANG["orderpaymentterm" . $paymentTerm] . "<br>\n";
                        if ($allowqty && 1 < $qty) {
                            $totalToday = $productdata["pricing"]["totaltoday"];
                            $adminemailitems .= $_LANG["quantity"] . ": " . $qty . "<br>\n" . $_LANG["invoicestotal"] . ": " . (string) $totalToday . "<br>\n";
                        }
                        $adminemailitems .= "<br>\n";
                    }
                }
            ...
            if ($addons) {
                foreach ($addons as $addonid) {
                    ...
                    $addonsarray[] = array("addonid" => $addonid, "name" => $addon_name, "pricingtext" => $pricing_text, "setup" => 0 < $addon_setupfee ? new WHMCS\View\Formatter\Price($addon_setupfee * $qty, $currency) : "", "recurring" => new WHMCS\View\Formatter\Price($addon_recurring, $currency), "isRecurring" => $addon_isRecurring, "billingcycle" => $addon_billingcycle, "billingcyclefriendly" => Lang::trans("orderpaymentterm" . $addon_billingcycle), "totaltoday" => new WHMCS\View\Formatter\Price($addon_total_today, $currency), "taxed" => $addon_tax);
    ...
    if (is_array($cartAddons)) {
        foreach ($cartAddons as $key => $addon) {
            ...
            $addonsarray[] = array("addonid" => $addonid, "name" => $addon_name, "productname" => $productname, "domainname" => $domainname, "pricingtext" => $pricing_text, "setup" => 0 < $addon_setupfee ? new WHMCS\View\Formatter\Price($addon_setupfee, $currency) : "", "totaltoday" => new WHMCS\View\Formatter\Price($addon_setupfee + $addon_recurring, $currency), "recurring" => new WHMCS\View\Formatter\Price($addon_recurring, $currency), "isRecurring" => $addon_isRecurring, "billingcycle" => $addon_billingcycle, "billingcyclefriendly" => Lang::trans("orderpaymentterm" . $addon_billingcycle), "taxed" => $addon_tax);
        }
    }
    ...
    if (is_array($cartDomains)) {
        ...
        foreach ($cartDomains as $key => $domain) {
            ...
            $tempdomains[$key] = array("type" => $domaintype, "domain" => $domainname, "regperiod" => $regperiod, "yearsLanguage" => $regperiod == 1 ? Lang::trans("orderForm.year") : Lang::trans("orderForm.years"), "shortYearsLanguage" => $regperiod == 1 ? Lang::trans("orderForm.shortPerYear", array(":years" => $regperiod)) : Lang::trans("orderForm.shortPerYears", array(":years" => $regperiod)), "price" => $pricing_text, "totaltoday" => new WHMCS\View\Formatter\Price($domainprice, $currency), "renewprice" => $renewPrice, "prefixedRenewPrice" => $renewPrice->toPrefixed(), "renewalPeriod" => $renewalPeriod, "renewalPeriodYearsLang" => $renewalPeriod == 1 ? Lang::trans("orderForm.year") : Lang::trans("orderForm.years"), "shortRenewalYearsLanguage" => $renewalPeriod == 1 ? Lang::trans("orderForm.shortPerYear", array(":years" => $renewalPeriod)) : Lang::trans("orderForm.shortPerYears", array(":years" => $renewalPeriod)), "dnsmanagement" => $dnsmanagement, "emailforwarding" => $emailforwarding, "idprotection" => $idprotection, "eppvalue" => $domain["eppcode"], "premium" => $domain["isPremium"], "pricing" => !is_null($domainPriceOverride) ? array(1 => $pricing_text) : $pricing, "taxed" => (bool) WHMCS\Config\Setting::getValue("TaxDomains"));
    ...
    if ($checkout) {
        ...
        if (!$_SESSION["cart"]["geninvoicedisabled"]) {
            ...
            if (!WHMCS\Config\Setting::getValue("NoInvoiceEmailOnOrder")) {
...
function acceptOrder($orderid, $vars = array())
{
    ...
    $services = WHMCS\Service\Service::with("product")->where("orderid", $orderid)->where("domainstatus", WHMCS\Service\Status::PENDING);
    foreach ($services->get() as $service) {
        ...
        if ($autosetup && $module) {
            ...
            $service->domainStatus = WHMCS\Service\Status::ACTIVE;
            $service->save();
            if ($sendwelcome) {
                sendMessage("defaultnewacc", $service->id);
            }
...
function changeOrderStatus($orderid, $status, $cancelSubscription = false)
{
    ...
    if ($invoiceid) {
        if ($status == "Pending") {
            WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invoiceid)->where("status", WHMCS\Billing\Invoice::STATUS_CANCELLED)->update(array("status" => WHMCS\Billing\Invoice::STATUS_UNPAID, "date_cancelled" => "0000-00-00 00:00:00"));
        } else {
...
function getAddons($pid, array $addons = array())
{
    ...
    foreach ($orderAddons as $addon) {
        ...
        if (!$pricing && !WHMCS\Billing\Cycles::isFree($addon->billingCycle)) {
            continue;
        }
        $addonPricingString = "";
...
function getAvailableOrderPaymentGateways($forceAll = false)
{
    ...
    $currencyId = isset($_SESSION["currency"]) ? $_SESSION["currency"] : NULL;
    $currency = getCurrency($userId, $currencyId);
