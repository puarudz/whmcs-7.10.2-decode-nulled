...
function refundInvoicePayment($transid, $amount, $sendtogateway, $addascredit = "", $sendemail = true, $refundtransid = "", $reverse = false)
{
    ...
    if ($addascredit) {
        ...
        if ($invoicetotalpaid - $invoicetotalrefunded - $amount <= 0) {
            WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invoiceid)->update(array("status" => WHMCS\Billing\Invoice::STATUS_REFUNDED, "date_refunded" => WHMCS\Carbon::now()->toDateTimeString()));
...
function refundCreditOnStatusChange($invoiceId, $status = "Fraud")
{
    ...
    if (0 < $creditAmount) {
        ...
        $client->credit += $creditAmount;
        $client->save();
        WHMCS\Database\Capsule::table("tblcredit")->insert(array("clientid" => $userId, "date" => WHMCS\Carbon::now()->format("Y-m-d"), "description" => "Credit Removed - Reason: Order status changed to " . $status . " - Invoice #" . $invoiceId, "amount" => $creditAmount));
