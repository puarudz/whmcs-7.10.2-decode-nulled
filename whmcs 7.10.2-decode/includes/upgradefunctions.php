...
function SumUpPackageUpgradeOrder($id, $newproductid, $newproductbillingcycle, $promocode, $paymentmethod = "", $checkout = "")
{
    ...
    $oldProductBillingCycle = $billingcycle;
    if ($promocode) {
        ...
        if (is_array($validatedPromoData)) {
            ...
            $promodata = array("type" => $type, "value" => $value);
        ...
        if (empty($promodata)) {
            $lifetimePromoData = get_query_vals("tblpromotions", "type,value", array("lifetimepromo" => 1, "recurring" => 1, "code" => $promocode));
    ...
    if (in_array($oldProductBillingCycle, array(WHMCS\Billing\Cycles::DISPLAY_FREE, WHMCS\Billing\Cycles::DISPLAY_ONETIME))) {
    ...
    foreach ($configoptions as $key => $configoption) {
        ...
        if (($optiontype == 1 || $optiontype == 2) && $new_selectedvalue != $old_selectedvalue || ($optiontype == 3 || $optiontype == 4) && $new_selectedqty != $old_selectedqty) {
            ...
            if ($checkout) {
                ...
                $upgradeid = insert_query("tblupgrades", array("type" => "configoptions", "userid" => WHMCS\Session::get("uid"), "date" => "now()", "relid" => $id, "originalvalue" => (string) $configid . "=>" . $db_orig_value, "newvalue" => $db_new_value, "amount" => $amountwithdiscount, "recurringchange" => $difference, "status" => "Pending", "paid" => $paid));
                $_SESSION["upgradeids"][] = $upgradeid;
                $hookReturns = run_hook("PreUpgradeCheckout", array("clientId" => (int) WHMCS\Session::get("uid"), "upgradeId" => $upgradeid, "serviceId" => $id, "amount" => $amountdue, "discount" => $discount));
                foreach ($hookReturns as $hookReturn) {
                    if (is_array($hookReturn)) {
...
function doUpgrade($upgradeid)
{
    ...
    if ($type == "package") {
        ...
        $updateData = array("packageid" => $newpackageid, "billingcycle" => $newbillingcycle);
        if ($changevalue === "amount") {
            $updateData["amount"] = recalcRecurringProductPrice($relid, "", $newpackageid, $newbillingcycle);
        } else {
            $updateData[(string) $changevalue] = "+=" . $recurringchange;
        }
        update_query("tblhosting", $updateData, array("id" => $relid));
