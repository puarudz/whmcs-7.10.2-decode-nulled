...
function getClientsStats($userid, WHMCS\User\Client $client = NULL)
{
    ...
    $invoicesData = WHMCS\Database\Capsule::table("tblinvoices")->where("tblinvoices.userid", $userid)->where("status", "Unpaid")->leftJoin("tblaccounts", "tblaccounts.invoiceid", "=", "tblinvoices.id")->whereNotIn("tblinvoices.id", $invoiceTypeItemInvoiceIds)->groupBy("tblinvoices.id")->get(array(WHMCS\Database\Capsule::raw("IFNULL(total, 0) as total"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountin), 0) as amount_in"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountout), 0) as amount_out")));
    ...
    $overDueInvoices = WHMCS\Database\Capsule::table("tblinvoices")->where("tblinvoices.userid", $userid)->where("status", "Unpaid")->where("duedate", "<", WHMCS\Carbon::today()->toDateTimeString())->leftJoin("tblaccounts", "tblaccounts.invoiceid", "=", "tblinvoices.id")->whereNotIn("tblinvoices.id", $invoiceTypeItemInvoiceIds)->groupBy("tblinvoices.id")->get(array(WHMCS\Database\Capsule::raw("IFNULL(total, 0) as total"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountin), 0) as amount_in"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountout), 0) as amount_out")));
    ...
    $invoicesData = WHMCS\Database\Capsule::table("tblinvoices")->where("tblinvoices.userid", $userid)->where("status", "Draft")->leftJoin("tblaccounts", "tblaccounts.invoiceid", "=", "tblinvoices.id")->whereNotIn("tblinvoices.id", $invoiceTypeItemInvoiceIds)->groupBy("tblinvoices.id")->get(array(WHMCS\Database\Capsule::raw("IFNULL(total, 0) as total"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountin), 0) as amount_in"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountout), 0) as amount_out")));
    $stats["numDraftInvoices"] = count($invoicesData);
    $invoicesData = collect($invoicesData);
    $stats["draftInvoicesBalance"] = formatCurrency($invoicesData->sum(function ($invoiceData) {
        return $invoiceData->total - $invoiceData->amount_in + $invoiceData->amount_out;
    }));
    $invoiceStatus = array(WHMCS\Billing\Invoice::STATUS_UNPAID => array("invoice_count" => 0, "total" => 0, "credit" => 0), WHMCS\Billing\Invoice::STATUS_PAID => array("invoice_count" => 0, "total" => 0, "credit" => 0), WHMCS\Billing\Invoice::STATUS_CANCELLED => array("invoice_count" => 0, "total" => 0, "credit" => 0), WHMCS\Billing\Invoice::STATUS_REFUNDED => array("invoice_count" => 0, "total" => 0, "credit" => 0), WHMCS\Billing\Invoice::STATUS_COLLECTIONS => array("invoice_count" => 0, "total" => 0, "credit" => 0), WHMCS\Billing\Invoice::STATUS_PAYMENT_PENDING => array("invoice_count" => 0, "total" => 0, "credit" => 0));
...
function addClient($firstname, $lastname, $companyname, $email, $address1, $address2, $city, $state, $postcode, $country, $phonenumber, $password, $securityqid = 0, $securityqans = "", $sendemail = true, array $additionalData = array(), $uuid = "", $isAdmin = false, $marketingOptIn = NULL)
{
    ...
    $table = "tblclients";
    $array = array("uuid" => $uuid, "firstname" => $firstname, "lastname" => $lastname, "companyname" => $companyname, "email" => $email, "address1" => $address1, "address2" => $address2, "city" => $city, "state" => $state, "postcode" => $postcode, "country" => $country, "phonenumber" => $phonenumber, "password" => $password_hash, "lastlogin" => "now()", "securityqid" => $securityqid, "securityqans" => encrypt($securityqans), "ip" => $remote_ip, "host" => $fullhost, "status" => "Active", "datecreated" => "now()", "language" => $_SESSION["Language"], "currency" => $currency["id"], "email_verified" => 0, "email_preferences" => json_encode(WHMCS\User\Client::$emailPreferencesDefaults));
    if (WHMCS\User\Client::where("email", $email)->first()) {
    ...
    if (defined("CLIENTAREA")) {
        $_SESSION["uid"] = $uid;
        $_SESSION["upw"] = WHMCS\Authentication\Client::generateClientLoginHash($uid, NULL, $password_hash, $email);
...
function cancelUnpaidInvoicebyProductID($serviceid, $userid = "")
{
    ...
    while ($data = mysql_fetch_array($result)) {
        ...
        if ($itemcount == 1) {
            WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invoiceid)->update(array("status" => WHMCS\Billing\Invoice::STATUS_CANCELLED, "date_cancelled" => WHMCS\Carbon::now()->toDateTimeString()));
    ...
    if ($addonIds) {
        ...
        foreach ($invoiceItems as $invoiceItem) {
            ...
            if ($itemCount == 1) {
                WHMCS\Database\Capsule::table("tblinvoices")->where("id", $invoiceItem->invoiceid)->update(array("status" => WHMCS\Billing\Invoice::STATUS_CANCELLED, "date_cancelled" => WHMCS\Carbon::now()->toDateTimeString()));
...
{
    $bankDetails = array("bankname" => NULL, "banktype" => NULL, "bankacct" => NULL, "bankcode" => NULL, "gatewayid" => NULL);
    $client = WHMCS\User\Client::find($userId);
    if (!$client) {
        return $bankDetails;
    }
...
function getClientsBankDetails($userId)
{
    ...
    if (!array_key_exists($userId, $users)) {
        ...
        $clientInfo = WHMCS\Database\Capsule::table("tblclients")->where("id", $userId)->first(array("bankname", "banktype", WHMCS\Database\Capsule::raw("AES_DECRYPT(bankcode, '" . $aesHash . "') as bankcode"), WHMCS\Database\Capsule::raw("AES_DECRYPT(bankacct, '" . $aesHash . "') as bankacct"), "gatewayid"));
