        ...
        if (!$deptid) {
            ...
            if ($to == $email) {
                ...
                if ($adminid) {
                    ...
                    $userid = WHMCS\User\Client::whereIn("status", array(WHMCS\User\Client::STATUS_ACTIVE, WHMCS\User\Client::STATUS_INACTIVE))->where("email", $email)->value("id");
                    $ccemail = $contactid = "";
                    $from = array();
                    if (!$userid) {
                            ...
                            if ($tid) {
                                ...
                                if ($pipenonregisteredreplyonly && !$userid) {
                                    ...
                                    if ($deptpiperepliesonly) {
                                        ...
                                        if (0 < count($cc)) {
                                            static $supportDepartmentEmails = NULL;
                                            if (is_null($supportDepartmentEmails)) {
                                                $supportDepartmentEmails = WHMCS\Database\Capsule::table("tblticketdepartments")->pluck("email");
                                            }
...
{
    static $uploadedAttachments = NULL;
