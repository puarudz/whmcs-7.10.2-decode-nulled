...
if (!in_array($moduleType, $supportedModuleTypes)) {
    ...
    if (!in_array($moduleName, $moduleInterface->getList())) {
        ...
        try {
            ...
            $apiresults = array("result" => "error", "message" => "Module configuration update not supported by module type.");
            return NULL;
        } catch (WHMCS\Exception\Module\NotActivated $e) {
            $apiresults = array("result" => "error", "message" => "Module Configuration Update Failed: " . $e->getMessage());
            return NULL;
        } catch (Exception $e) {
