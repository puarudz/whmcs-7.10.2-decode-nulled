...
while ($data = mysql_fetch_array($result)) {
    ...
    $apiresults["tickets"]["ticket"][] = array("id" => $id, "tid" => $tid, "deptid" => $deptid, "userid" => $userid, "name" => $name, "email" => $email, "cc" => $cc, "c" => $c, "date" => $date, "subject" => $subject, "status" => $status, "priority" => $priority, "admin" => $admin, "attachment" => $attachment, "attachments" => $attachments, "attachments_removed" => $attachmentsRemoved, "lastreply" => $lastreply, "flag" => $flag, "service" => $service);
}
$responsetype = "xml";
