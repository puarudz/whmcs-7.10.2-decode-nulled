            ...
            if (in_array($type, array(strtolower(WHMCS\Payment\PayMethod\Model::TYPE_CREDITCARD_LOCAL), strtolower(WHMCS\Payment\PayMethod\Model::TYPE_CREDITCARD_REMOTE_MANAGED)))) {
                ...
                switch ($workFlowType) {
                    ...
                    case WHMCS\Module\Gateway::WORKFLOW_MERCHANT:
                        if (!(new WHMCS\Gateways())->isLocalCreditCardStorageEnabled(false)) {
                            $apiresults = array("result" => "error", "message" => "No Local Credit Card Payment Gateways Enabled");
                            return NULL;
                        }
                ...
                if ($gatewayInterface && !$gatewayInterface->supportsLocalBankDetails()) {
                    $apiresults = array("result" => "error", "message" => "Selected gateway does not support local bank accounts.");
                    return NULL;
                }
