        ...
        if (!$clientInformationAvailable) {
            ...
            if ($initialVerificationId != $verificationId && !$clientInformation->emailVerified) {
                ...
                if ($transientDataName) {
                    $transientNameParts = explode(":", $transientDataName);
                    if (empty($transientNameParts[0]) || (int) $transientNameParts[0] !== (int) $clientInformation->id) {
                        WHMCS\Session::delete("uid");
                        WHMCS\Session::set("initialVerificationId", $verificationId);
                    } else {
...
if ($action == "") {
    ...
    if (checkContactPermission("invoices", true)) {
        ...
        $invoices = WHMCS\Database\Capsule::table("tblinvoices")->where("tblinvoices.userid", $clientId)->where("status", "Unpaid")->where("duedate", "<", WHMCS\Carbon::now()->toDateString())->whereNotIn("tblinvoices.id", $invoiceTypeItemInvoiceIds)->leftJoin("tblaccounts", "tblaccounts.invoiceid", "=", "tblinvoices.id")->groupBy("tblinvoices.id")->get(array(WHMCS\Database\Capsule::raw("IFNULL(total, 0) as total"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountin), 0) as amount_in"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountout), 0) as amount_out")));
        $invoiceCount = count($invoices);
        if ($invoiceCount) {
            $invoices = collect($invoices);
            $msg = Lang::trans("clientHomePanels.overdueInvoicesMsg", array(":numberOfInvoices" => $invoiceCount, ":balanceDue" => formatCurrency($invoices->sum(function ($invoice) {
            ...
            $invoices = WHMCS\Database\Capsule::table("tblinvoices")->where("tblinvoices.userid", $clientId)->where("status", "Unpaid")->whereNotIn("tblinvoices.id", $invoiceTypeItemInvoiceIds)->leftJoin("tblaccounts", "tblaccounts.invoiceid", "=", "tblinvoices.id")->groupBy("tblinvoices.id")->get(array(WHMCS\Database\Capsule::raw("IFNULL(SUM(DISTINCT total), 0) as total"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountin), 0) as amount_in"), WHMCS\Database\Capsule::raw("IFNULL(SUM(amountout), 0) as amount_out")));
            $invoiceCount = count($invoices);
            if ($invoiceCount) {
                $invoices = collect($invoices);
                $msg = Lang::trans("clientHomePanels.unpaidInvoicesMsg", array(":numberOfInvoices" => $invoiceCount, ":balanceDue" => formatCurrency($invoices->sum(function ($invoice) {
                    return $invoice->total - $invoice->amount_in + $invoice->amount_out;
                }))));
    ...
    if ($action == "details") {
        ...
        if ($save) {
            ...
            if ($e) {
                ...
                if (!$contactId) {
                    WHMCS\Session::set("upw", WHMCS\Authentication\Client::generateClientLoginHash(WHMCS\Session::get("uid"), WHMCS\Session::get("cid"), $legacyClient->getClientModel()->password, $legacyClient->getClientModel()->email));
                }
        ...
        if ($action == "contacts") {
            ...
            if ($submit) {
                ...
                if (!$errormessage) {
                    ...
                    $array = db_build_update_array(array("firstname", "lastname", "companyname", "email", "address1", "address2", "city", "state", "postcode", "country", "phonenumber", "subaccount", "permissions", "tax_id"), "implode");
                    foreach (WHMCS\Mail\Emailer::CLIENT_EMAILS as $emailType) {
                        if ($emailType === WHMCS\Mail\Emailer::EMAIL_TYPE_AFFILIATE) {
                            continue;
                        }
                        $array[$emailType . "emails"] = App::getFromRequest("email_preferences", $emailType);
                    }
                    ...
                    if ($loggedInContactId && $loggedInContactId == $id) {
                        if (isset($array["password"])) {
                            $passwordHash = $array["password"];
                        } else {
                            $passwordHash = $oldcontactdata["password"];
                        }
                        WHMCS\Session::set("upw", WHMCS\Authentication\Client::generateClientLoginHash(WHMCS\Session::get("uid"), WHMCS\Session::get("cid"), $passwordHash, $array["email"]));
                    }
                    if (!$subaccount) {
            ...
            foreach (WHMCS\Mail\Emailer::CLIENT_EMAILS as $emailType) {
                ...
                $emailPreferences[$emailType] = App::get_req_var_if($e, "email_preferences", $contact_data, $emailType . "emails", $emailType);
            }
            $ca->assign("emailPreferences", $emailPreferences);
                ...
                if ($action == "creditcard") {
                    ...
                    if ($action == "changepw") {
                        ...
                        if ($submit) {
                            ...
                            if ($contactId) {
                                $result = select_query("tblcontacts", "password,email", array("id" => $contactId, "userid" => $userId));
                            } else {
                                $result = select_query("tblclients", "password,email", array("id" => $userId));
                            }
                            ...
                            if (!$validate->hasErrors()) {
                                ...
                                WHMCS\Session::set("upw", WHMCS\Authentication\Client::generateClientLoginHash($userId, $contactId, $passwordToSave, $userEmail));
                                    ...
                                    if (WHMCS\UsageBilling\MetricUsageSettings::isCollectionEnable()) {
                                        foreach ($metrics as $serviceMetric) {
                                            ...
                                            foreach ($pricingSchema as $bracket) {
                                                ...
                                                $pricing[] = array("from" => $floor, "price_per_unit" => formatCurrency($currencyPrice->monthly));
                                            ...
                                            $metricStats[] = array("type" => $serviceMetric->type(), "systemName" => $serviceMetric->systemName(), "displayName" => $serviceMetric->displayName(), "unitName" => $units->perUnitName(1), "includedQuantity" => $freeLimit, "includedQuantityUnits" => $units->perUnitName($freeLimit), "lowestPrice" => $lowestPrice, "pricingSchema" => array("info" => Lang::trans($baseLangKey . ".info"), "detail" => Lang::trans($baseLangKey . ".detail")), "pricing" => $pricing, "postPeriodValue" => $postPeriodValue, "postPeriodDateRange" => $postPeriodDateRange, "currentValue" => $currentValue, "lastUpdated" => $lastUpdated);
                                        }
                                    }
                                                                                        ...
                                                                                        foreach ($invoiceids as $invoiceid) {
                                                                                            ...
                                                                                            if ($invoiceid) {
                                                                                                ...
                                                                                                if ($geninvoice) {
                                                                                                    ...
                                                                                                    insert_query("tblinvoiceitems", array("userid" => $legacyClient->getID(), "type" => "Invoice", "relid" => $invoiceid, "description" => $description, "amount" => $thistotal, "duedate" => "now()", "paymentmethod" => $paymentmethod));
                                                                                                }
