...
class Orders extends TableModel
{
    ...
    private function changeStatus($orderid, $status)
    {
        ...
        $invoiceid = $data["invoiceid"];
        if ($status == "Pending") {
            Database\Capsule::table("tblinvoices")->where("id", $invoiceid)->where("status", Billing\Invoice::STATUS_CANCELLED)->update(array("status" => Billing\Invoice::STATUS_UNPAID, "date_cancelled" => "0000-00-00 00:00:00"));
        } else {
            Database\Capsule::table("tblinvoices")->where("status", Billing\Invoice::STATUS_UNPAID)->where("id", $invoiceid)->update(array("status" => Billing\Invoice::STATUS_CANCELLED, "date_cancelled" => Carbon::now()->toDateTimeString()));
