...
class Auth
{
    ...
    public static function persistClientSession()
    {
        ...
        if (defined("CLIENTAREA") && !isset($_SESSION["uid"]) && isset($_COOKIE["WHMCSUser"])) {
            ...
            if (is_numeric($userId) && (empty($contactId) || is_numeric($contactId)) && !empty($cookieHash)) {
                ...
                $data = $table->first(array("id", "password", "email"));
                $loginhash = Authentication\Client::generateClientLoginHash($userId, $contactId, $data->password, $data->email);
        ...
        if (isset($_SESSION["uid"])) {
            ...
            if ($sessionContactId) {
                ...
                $result = select_query("tblcontacts", "tblcontacts.id, tblcontacts.password, tblcontacts.email", $where, "", "", "", "tblclients ON tblclients.id = tblcontacts.userid", $handle);
            } else {
                $where = array("id" => (int) $sessionUserId);
                if (empty($sessionAdminId)) {
                    $where["status"] = array("sqltype" => "IN", "values" => array("Active", "Inactive"));
                }
                $result = select_query("tblclients", "id, password, email", $where, "", "", "", "", $handle);
            }
            ...
            if ($dbId) {
                $computedHash = Authentication\Client::generateClientLoginHash($sessionUserId, $sessionContactId, $dbPassword, $data["email"]);
    ...
    public function authenticateClientFromToken(ApplicationLink\AccessToken $token)
    {
        load_hooks();
        Session::rotate();
        $login_uid = $login_cid = 0;
        $client = $contact = null;
        $user = $token->user;
        if ($user) {
            if ($user instanceof User\Client\Contact) {
                $client = $user->client;
                $contact = $user;
            } else {
                $client = $user;
            }
            if (!$client || !$client->isAllowedToAuthenticate()) {
                return false;
            }
            $login_uid = $client->id;
            if ($contact) {
        ...
        if ($login_uid) {
            ...
            $_SESSION["upw"] = Authentication\Client::generateClientLoginHash($login_uid, $login_cid, $user->passwordHash, $user->email);
            ...
            run_hook("ClientLogin", array("userid" => $login_uid, "contactid" => $login_cid));
            return true;
        }
        return false;
    }
    public function cleanRedirectUri($uri)
    {
        ...
        if (strpos($uri, "?") !== false) {
            ...
            if (preg_match("/rp=([^&]+)/", $uriSegments[1], $matches)) {
                $routePath = $matches[1];
                $baseUrl = defined("ADMINAREA") ? Utility\Environment\WebHelper::getAdminBaseUrl() : Utility\Environment\WebHelper::getBaseUrl();
