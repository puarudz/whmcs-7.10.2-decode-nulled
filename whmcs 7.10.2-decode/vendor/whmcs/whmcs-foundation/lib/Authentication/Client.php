...
class Client
{
    ...
    protected function setSessionIds($user)
    {
        ...
        $userSessionHash = static::generateClientLoginHash($userId, $contactId, $user->passwordHash, $user->email);
    ...
    public static function generateClientLoginHash($userId, $contactId, $passwordHash, $email)
    {
        ...
        $delimiter = "|";
        return sha1($userId . $delimiter . $contactId . $delimiter . $passwordHash . $delimiter . $email . $delimiter . $userIp . $delimiter . $hashSalt);
    }
}
