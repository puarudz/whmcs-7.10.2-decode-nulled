...
class Register extends \Illuminate\Database\Eloquent\Model implements RegisterInterface
{
    ...
    {
        return $query->whereBetween("created_at", array($on->startOfDay()->toDateTimeString(), $on->endOfDay()->toDateTimeString()))->where("namespace_id", $id)->orderBy("created_at", "asc")->orderBy("id", "asc");
    }
