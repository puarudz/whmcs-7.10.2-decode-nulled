...
class Server extends AbstractModule
{
    ...
    protected function buildServiceParams(\WHMCS\Service\Service $serviceData)
    {
        ...
        foreach ($customFields as $fieldName => $fieldValue) {
            ...
            $customFields[$fieldName] = \WHMCS\Input\Sanitize::encode(\WHMCS\Input\Sanitize::stripTags(\WHMCS\Input\Sanitize::decode($fieldValue)));
        }
    ...
    public function call($function, array $params = array())
    {
        ...
        if (!$addonId && $manageUsageOnTerminate) {
        ...
        if ($manageUsageOnTerminate && is_string($actionResult) && strtolower($actionResult) == "success") {
