...
class Gateway extends AbstractModule
{
    ...
    {
        return \WHMCS\Database\Capsule::table("tblpaymentgateways")->distinct("gateway")->where("setting", "type")->where("value", self::GATEWAY_CREDIT_CARD)->orderBy("gateway")->pluck("gateway");
    }
    ...
    public function call($function, array $params = array())
    {
        ...
        if ($clientBeforeCall && in_array($function, array("capture", "3dsecure", "post_checkout"))) {
    ...
    public function activate(array $parameters = array())
    {
        ...
        if (!in_array($gatewayType, array(self::GATEWAY_BANK, self::GATEWAY_CREDIT_CARD, self::GATEWAY_THIRD_PARTY))) {
