...
class ClientArea extends Http\Message\AbstractViewableResponse
{
    ...
    public function initPage()
    {
        ...
        if ($this->isLoggedIn()) {
            ...
            if ($contactId = (int) Session::get("cid")) {
                $contact = User\Client\Contact::find($contactId)->where("userid", $clientId)->first();
                $loggedinuser = array("contactid" => $contact->id, "firstname" => $contact->firstname, "lastname" => $contact->lastname, "email" => $contact->email);
    ...
    {
        $contact = User\Client\Contact::find(Session::get("cid"));
        $contactPermissions = $contact->permissions;
        foreach ($contactPermissions as $key => $permission) {
            $contactPermissions[$key] = \Lang::trans("subaccountperms" . $permission);
        }
        $this->setDisplayTitle(\Lang::trans("accessdenied"));
        $this->assign("allowedpermissions", $contactPermissions);
        $this->assign("requiredpermission", $requiredPermission);
        $this->setTemplate("contactaccessdenied");
        return $this;
    }
}
