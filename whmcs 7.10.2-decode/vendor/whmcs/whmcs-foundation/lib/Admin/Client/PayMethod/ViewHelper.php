    ...
    protected function clientProfileHtml(\WHMCS\User\Client $client, $template, array $vars = array())
    {
        ...
        if ($resolver->hasGatewaysSupportingManage()) {
            ...
            foreach ($cardGatewayModules as $name => $module) {
                if ($module->getWorkflowType() == \WHMCS\Module\Gateway::WORKFLOW_NOLOCALCARDINPUT || $module->isTokenised() && !($module->functionExists("storeremote") || $module->functionExists("remoteinput"))) {
                    continue;
                }
