...
class Process
{
    ...
    public static function sync(SyncItem $syncItem, ServiceItem $serviceItem, $serverId)
    {
        ...
        if ($syncItem->getProduct() != $serviceItem->getProduct()) {
            ...
            $newProduct = \WHMCS\Database\Capsule::table("tblproducts")->where("servertype", $currentServiceModule)->where(function ($query) use($syncItem) {
                $fieldValue = explode("_", $syncItem->getProduct(), 2);
                $query->where($syncItem->getProductField(), $syncItem->getProduct())->orWhere($syncItem->getProductField(), $fieldValue[1]);
            })->first();
            if ($newProduct) {
