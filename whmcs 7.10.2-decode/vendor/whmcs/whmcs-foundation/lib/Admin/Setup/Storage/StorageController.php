...
class StorageController
{
    ...
    public function index(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $viewAssetSettings = array();
        $dbAssetSettings = \WHMCS\File\Configuration\FileAssetSetting::with(array("configuration", "migrateToConfiguration"))->get();
        $dbAssetSettings = $dbAssetSettings->sort(function (\WHMCS\File\Configuration\FileAssetSetting $setting1, \WHMCS\File\Configuration\FileAssetSetting $setting2) {
            return strcmp(\AdminLang::trans("storage.assetTypes." . $setting1->asset_type), \AdminLang::trans("storage.assetTypes." . $setting2->asset_type));
        });
        foreach ($dbAssetSettings as $assetSetting) {
            ...
            $viewAssetSettings[] = array_merge($assetSetting->toArray(), array("can_migrate" => \WHMCS\File\FileAsset::canMigrate($assetSetting->asset_type), "migration" => $migration));
        }
        ...
        $params = array("assetSettings" => $viewAssetSettings, "storageConfigurations" => $storageConfigurations, "providers" => $providers, "csrfToken" => generate_token("plain"), "action" => $action);
