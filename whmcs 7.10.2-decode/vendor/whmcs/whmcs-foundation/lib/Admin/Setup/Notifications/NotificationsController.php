...
class NotificationsController
{
    ...
    public function listNotifications(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        try {
            foreach (\WHMCS\Notification\Rule::orderBy("updated_at", "desc")->get() as $rule) {
                ...
                foreach ($rule->events as $eventKey) {
    ...
    public function saveRule(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $providerInterface = $provider->initObject();
        foreach ($providerInterface->notificationSettings() as $field => $values) {
        ...
        $successMessage = $ruleId ? \AdminLang::trans("notifications.ruleUpdatedSuccessfully") : \AdminLang::trans("notifications.ruleCreatedSuccessfully");
        if (method_exists($providerInterface, "postRuleSave")) {
            try {
                $message = "";
                $type = "";
                $providerInterface->postRuleSave($provider->settings, $rule->providerConfig);
            ...
            if ($message) {
                return new \WHMCS\Http\Message\JsonResponse(array("body" => "<div class=\"alert alert-" . $type . "\" role=\"alert\">" . $message . "</div>"));
            }
        }
        return new \WHMCS\Http\Message\JsonResponse(array("successMsgTitle" => "", "successMsg" => $successMessage, "dismiss" => true));
    }
