...
class TaxController
{
    public function saveSettings(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if (\WHMCS\Config\Setting::getValue("TaxAutoResetNumbering") != $autoResetNumbering) {
            \WHMCS\Config\Setting::setValue("TaxNextCustomInvoiceNumberResetTimestamp", "");
        }
        if (\WHMCS\Config\Setting::getValue("TaxAutoResetPaidNumbering") != $autoResetPaidNumbering) {
            \WHMCS\Config\Setting::setValue("SequentialInvoiceNumberValueResetTimestamp", "");
        }
        $changes = array();
