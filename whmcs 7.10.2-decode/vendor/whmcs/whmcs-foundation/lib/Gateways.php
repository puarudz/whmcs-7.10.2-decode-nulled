...
class Gateways
{
    ...
    public function getDisplayNames()
    {
        ...
        while ($data = mysql_fetch_array($result)) {
            ...
            try {
                $gatewayInterface = Module\Gateway::factory($data["gateway"]);
                $this->displaynames[$data["gateway"]] = $gatewayInterface->getDisplayName();
            } catch (\Exception $e) {
            }
        }
    ...
    {
        $groupedGateways = array("assisted" => array(), "nolocalcardinput" => array(), "merchant" => array(), "remote" => array(), "thirdparty" => array(), "token" => array());
        $query = Database\Capsule::table("tblpaymentgateways as gw1")->where("gw1.setting", "type")->where("gw1.value", Module\Gateway::GATEWAY_CREDIT_CARD)->leftJoin("tblpaymentgateways as gw2", "gw1.gateway", "=", "gw2.gateway")->where("gw2.setting", "visible");
    ...
    {
        foreach ($this->getAvailableGatewayInstances() as $gatewayInstance) {
            if ($gatewayInstance->getWorkflowType() !== Module\Gateway::WORKFLOW_NOLOCALCARDINPUT) {
                return true;
            }
        }
        return false;
