...
class Invoice
{
    ...
    {
        return array(Billing\Invoice::STATUS_DRAFT, Billing\Invoice::STATUS_UNPAID, Billing\Invoice::STATUS_PAID, Billing\Invoice::STATUS_CANCELLED, Billing\Invoice::STATUS_REFUNDED, Billing\Invoice::STATUS_COLLECTIONS, Billing\Invoice::STATUS_PAYMENT_PENDING);
    }
    ...
    {
        return $paymentSuccessful == true && in_array($this->getData("status"), array(Billing\Invoice::STATUS_UNPAID, Billing\Invoice::STATUS_PAYMENT_PENDING)) && $this->isAssignedGatewayWithDelayedCallbacks();
    }
}
