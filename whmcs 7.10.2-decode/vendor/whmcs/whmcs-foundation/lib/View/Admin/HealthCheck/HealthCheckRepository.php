...
class HealthCheckRepository
{
    ...
    {
        $vendorCheckUrl = \App::getSystemURL(false) . "/vendor/composer/LICENSE";
        $pathsToCheck = array($vendorCheckUrl => array("directory" => "/vendor"));
        $accessiblePaths = array();
        $downloadClient = new \GuzzleHttp\Client(array("defaults" => array("timeout" => 5, "allow_redirects" => false)));
        foreach ($pathsToCheck as $url => $path) {
            $requests[] = $downloadClient->createRequest("GET", $url);
        }
        $pool = new \GuzzleHttp\Pool($downloadClient, $requests, array("pool_size" => 5, "complete" => function (\GuzzleHttp\Event\CompleteEvent $event) use($pathsToCheck, &$accessiblePaths) {
            $pathData = $pathsToCheck[$event->getRequest()->getUrl()];
            $accessiblePaths[] = $pathData["directory"];
        }));
        $pool->wait();
        if (empty($accessiblePaths)) {
            ...
            $logLevel = \Psr\Log\LogLevel::ERROR;
            $body = \AdminLang::trans("healthCheck.sensitiveDirsAccessible", array(":accessiblePaths" => implode("", array_map(function ($path) {
                return "<li>" . $path . "</li>";
    ...
    protected function checkForUnsupportedWebServer()
    {
        if (\WHMCS\Environment\WebServer::supportsHtaccess()) {
            $logLevel = \Psr\Log\LogLevel::NOTICE;
            $bodyHtml = \AdminLang::trans("healthCheck.supportedWebserver", array(":server" => \WHMCS\Environment\WebServer::getServerFamily()));
        } else {
            $logLevel = \Psr\Log\LogLevel::WARNING;
            $bodyHtml = \AdminLang::trans("healthCheck.unsupportedWebserver", array(":server" => \WHMCS\Environment\WebServer::getServerFamily(), ":envGuideline" => "https://docs.whmcs.com/System_Environment_Guide", ":learnMore" => "https://docs.whmcs.com/Further_Security_Steps#Enforce_htaccess_directives"));
        }
    ...
    protected function checkPhpTimezone()
    {
        ...
        return new HealthCheckResult("phpSettings", "PHP", \AdminLang::trans("healthCheck.phpTimezone"), $tzValid ? \Psr\Log\LogLevel::NOTICE : \Psr\Log\LogLevel::ERROR, $tzValid ? \AdminLang::trans("healthCheck.phpTimezoneOk") : \AdminLang::trans("healthCheck.phpTimezoneNotSet", array(":href" => "https://docs.whmcs.com/Changing_Timezone")));
    }
