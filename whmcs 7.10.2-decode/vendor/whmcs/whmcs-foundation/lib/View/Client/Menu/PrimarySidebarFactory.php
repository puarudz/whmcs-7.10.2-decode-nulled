...
class PrimarySidebarFactory extends \WHMCS\View\Menu\MenuFactory
{
    ...
    public function invoiceList()
    {
        ...
        $i = 1;
        $invoiceStatusCounts = array(\WHMCS\Billing\Invoice::STATUS_PAID => 0, \WHMCS\Billing\Invoice::STATUS_UNPAID => 0, \WHMCS\Billing\Invoice::STATUS_CANCELLED => 0, \WHMCS\Billing\Invoice::STATUS_REFUNDED => 0);
        ...
        if (0 < $invoiceStatusCounts["Unpaid"]) {
            ...
            $invoices = \WHMCS\Database\Capsule::table("tblinvoices")->where("tblinvoices.userid", $clientId)->where("status", "Unpaid")->where("type", "!=", "Invoice")->join("tblinvoiceitems", "tblinvoiceitems.invoiceid", "=", "tblinvoices.id")->leftJoin("tblaccounts", "tblaccounts.invoiceid", "=", "tblinvoices.id")->groupBy("tblinvoices.id")->get(array(\WHMCS\Database\Capsule::raw("IFNULL(total, 0) as total"), \WHMCS\Database\Capsule::raw("IFNULL(SUM(amountin), 0) as amount_in"), \WHMCS\Database\Capsule::raw("IFNULL(SUM(amountout), 0) as amount_out")));
            $invoices = collect($invoices);
            $invoicesDueMessage = sprintf(\Lang::trans("invoicesduemsg"), $invoiceStatusCounts["Unpaid"], formatCurrency($invoices->sum(function ($invoice) {
