...
class SecondarySidebarFactory extends PrimarySidebarFactory
{
    ...
    public function ticketView()
    {
        ...
        if ($ticketData["attachment"]) {
            ...
            foreach ($attachment as $filename) {
                $attachments[] = array("replyid" => 0, "i" => $attachmentCount, "filename" => substr($filename, 7), "removed" => (bool) (int) $ticketData["attachments_removed"]);
                $attachmentCount++;
            }
        }
    ...
    public function orderFormView()
    {
        ...
        if (!is_null($client)) {
            ...
            $categoryChildren[] = array("name" => "Addons", "label" => \Lang::trans("cartproductaddons"), "uri" => "cart.php?gid=addons", "order" => $order * 10, "current" => $productGroupId == "addons");
        }
        $i = 1;
