...
class Client extends AbstractUser implements Contracts\ContactInterface, UserInterface
{
    ...
    {
        $billingContacts = array(array("id" => 0, "firstname" => $this->firstName, "lastname" => $this->lastName, "companyname" => $this->companyName, "email" => $this->email, "address1" => $this->address1, "address2" => $this->address2, "city" => $this->city, "state" => $this->state, "postcode" => $this->postcode, "country" => $this->country, "countryname" => $this->countryName, "phonenumber" => $this->phoneNumber));
    ...
    {
        \WHMCS\Database\Capsule::table("tblcredit")->insert(array("clientid" => $this->id, "date" => \WHMCS\Carbon::now()->toDateString(), "description" => $description, "amount" => $amount));
