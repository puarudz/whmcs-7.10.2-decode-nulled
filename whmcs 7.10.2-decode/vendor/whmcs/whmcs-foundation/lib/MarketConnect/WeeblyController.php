...
class WeeblyController
{
    public function index(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $ca->initPage();
        if ($isAdminPreview) {
            $all = (new ServicesFeed())->getEmulationOfConfiguredProducts("weebly");
        } else {
        ...
        foreach ($all as $key => $product) {
            ...
            if ($product->productKey == "weebly_free") {
                $litePlan = $product;
                unset($all[$key]);
            }
        }
    ...
    public function upgrade(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $ca->assign("captcha", new \WHMCS\Utility\Captcha());
        $ca->assign("captchaForm", \WHMCS\Utility\Captcha::FORM_LOGIN);
    ...
    protected function setUpgradeProductKeyByPlan($plan)
    {
        if (strtolower($plan) == "business") {
            ...
            if (strtolower($plan) == "pro") {
                ...
                $upgradePlanProductKey = "weebly_starter";
            }
        }
    ...
    protected function setUpgradeProductKeyByServiceId($serviceId)
    {
        ...
        if ($currentProductKey == "weebly_free" || $currentProductKey == "weebly_lite") {
            $upgradePlanProductKey = "weebly_starter";
        } else {
    ...
    protected function getUpgradeProductKey()
    {
        ...
        if (!in_array($upgradePlanProductKey, array("weebly_starter", "weebly_pro", "weebly_business"))) {
            $upgradePlanProductKey = "weebly_starter";
        }
