    ...
    protected static function getMenuItemsChildren(&$i = 0, $order = 1000)
    {
        ...
        foreach (Service::active()->get() as $service) {
            if ($service->setting("general.activate-landing-page") !== false) {
                ...
                $name = $service->name;
                $children[] = array("name" => $name, "label" => \Lang::trans("navMarketConnectService." . $name), "uri" => routePath(isset($routeMap[$name]) ? "store-" . $routeMap[$name] . "-index" : "store-" . $name . "-index"), "order" => $order + $iVar * 10);
                $i = $iVar + 1;
            }
        }
        return $children;
