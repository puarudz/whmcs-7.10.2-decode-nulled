...
class AdminController
{
    ...
    public function enableProduct(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if ($productModel->paymentType === "free" && $otherProducts->count() === 0) {
            return array("error" => "At least one paid product must be enabled to use the free product offering");
        }
    ...
    public function getServices(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        foreach ($products as $product) {
            foreach ($product->services as $clientService) {
                ...
                $searchResults[] = array("id" => $clientService->id, "name" => $clientService->client->fullName, "domain" => $clientService->domain, "order" => $clientService->serviceProperties->get("Order Number"), "date" => fromMySQLDate($clientService->registrationDate));
            }
        }
        ...
        foreach ($moduleConfigurations as $moduleConfiguration) {
            ...
            foreach ($productAddon->serviceAddons as $addon) {
                ...
                $searchResults[] = array("id" => "a" . $addon->id, "name" => $addon->client->fullName, "domain" => $addon->service->domain, "order" => $addon->serviceProperties->get("Order Number"), "date" => fromMySQLDate($addon->registrationDate));
            }
        }
