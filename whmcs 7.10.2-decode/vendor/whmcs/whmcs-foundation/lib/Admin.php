...
class Admin
{
    ...
    public function __construct($reqpermission, $releaseSession = true)
    {
        ...
        $eula = new Utility\Eula();
        $request = Http\Message\ServerRequest::fromGlobals();
        if (!$eula->isEulaAccepted() && !$request->isXHR()) {
            $whmcs->redirectToRoutePath("admin-eula-required");
        }
        $currentTemplate = $this->adminTemplate;
        $templateLocation = $this->getTemplatePath() . $currentTemplate . "/header.tpl";
        if (!file_exists($templateLocation)) {
            $templates = new File\Directory($whmcs->get_admin_folder_name() . DIRECTORY_SEPARATOR . "templates");
            $availableTemplates = $templates->getSubdirectories();
            if (!empty($availableTemplates)) {
                $this->setAdminTemplate($availableTemplates[0]);
            }
            $admin = $this->getAdminUser();
            logActivity("Admin area template setting reset due to missing" . " template \"" . $currentTemplate . "\" - Admin ID: " . $admin->id . " - Admin Name: " . (string) $admin->fullName);
            redir("invalidtpl=" . $currentTemplate, "myaccount.php");
        }
        if ($releaseSession) {
            Session::release();
        }
    ...
    {
        $plugins = array("autosave", "print", "preview", "searchreplace", "autolink", "directionality", "visualblocks", "visualchars", "fullscreen", "image", "link", "media", "template", "code", "codesample", "table", "charmap", "hr", "pagebreak", "nonbreaking", "anchor", "insertdatetime", "advlist", "lists", "textcolor", "wordcount", "contextmenu", "colorpicker", "textpattern", "help");
        switch ($location) {
            case "emailTemplates":
            case "sendMessage":
                $routeParam = Utility\Image::IMAGE_EMAIL;
                break;
            case "knowledgebase":
                $routeParam = Utility\Image::IMAGE_KNOWLEDGEBASE;
                break;
            default:
                $uploadVariables = "";
                $plugins[] = "imagetools";
        ...
        if ($routeParam) {
            $plugins[] = "paste";
            $uploadUrl = routePath("admin-image-upload", $routeParam);
            $recentUploads = routePath("admin-image-recent-uploads", $routeParam);
            $uploadVariables = ",\n    images_upload_handler: function (blobInfo, success, failure) {\n        var xhr, formData;\n        \n        xhr = new XMLHttpRequest();\n        xhr.withCredentials = false;\n        xhr.open('POST', '" . $uploadUrl . "');\n        \n        xhr.onload = function() {\n            var json;\n            \n            if (xhr.status != 200) {\n                failure('HTTP Error: ' + xhr.status + ' - ' + xhr.responseText);\n                return;\n            }\n            \n            json = JSON.parse(xhr.responseText);\n            \n            if (!json || typeof json.location != 'string') {\n                failure('Invalid JSON: ' + xhr.responseText);\n                return;\n            }\n            \n            success(json.location);\n        };\n        \n        formData = new FormData();\n        formData.append('token', csrfToken);\n        formData.append('file', blobInfo.blob(), blobInfo.filename());\n        \n        xhr.send(formData);\n    },\n    automatic_uploads: true,\n    file_browser_callback_types: \"image\",\n    images_reuse_filename: true,\n    image_list: \"" . $recentUploads . "\"\n";
        }
    ...
    {
        $adminId = $this->getAdminID();
        Database\Capsule::table("tbladmins")->where("id", $adminId)->update(array("template" => $templateName));
    }
}
