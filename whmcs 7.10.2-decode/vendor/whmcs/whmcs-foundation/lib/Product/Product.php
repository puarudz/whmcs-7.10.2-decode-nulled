...
class Product extends \WHMCS\Model\AbstractModel
{
    ...
    public static function boot()
    {
        ...
        static::creating(function (Product $product) {
            if (!isset($product->welcomeEmailTemplateId)) {
                $emailTemplate = \WHMCS\Mail\Template::master()->where("name", self::DEFAULT_EMAIL_TEMPLATES[$product->type])->first();
                if ($emailTemplate) {
    ...
    public function scopeSsl(\Illuminate\Database\Eloquent\Builder $query)
    {
        ...
        return $query->where(function ($query) {
            $query->where("configoption1", "like", "rapidssl\\_%")->orWhere("configoption1", "like", "geotrust\\_%")->orWhere("configoption1", "like", "digicert\\_%");
        });
    }
    ...
    {
        $query = $this->scopeMarketConnect($query);
        return $query->where("configoption1", "like", "codeguard\\_%");
    }
    public function scopeMarketgoo(\Illuminate\Database\Eloquent\Builder $query)
    {
        $query = $this->scopeMarketConnect($query);
        return $query->where("configoption1", "like", "marketgoo\\_%");
    }
    ...
    {
        return $this->belongsToMany("WHMCS\\Admin\\Utilities\\Tools\\EmailMarketer", "tblemailmarketer_related_pivot", "product_id", "task_id")->withTimestamps();
    }
}
