...
class Emailer
{
    ...
    protected function getClientMergeData()
    {
        ...
        $sendToClient = true;
        if ($contact instanceof \WHMCS\User\Client && !\WHMCS\Config\Setting::getValue("DisableClientEmailPreferences") && $this->allowCc() && !$contact->getEmailPreference($this->getMessage()->getType())) {
            $sendToClient = false;
        }
        if ($sendToClient) {
