...
class Clients extends TableModel
{
    ...
    public function getClients($criteria = array())
    {
        ...
        $joinPayMethodTables = !empty($criteria["cctype"]) || !empty($criteria["cclastfour"]);
        ...
        $clients = array();
        $query = "SELECT DISTINCT tblclients.id,tblclients.firstname,tblclients.lastname,tblclients.companyname,tblclients.email,tblclients.datecreated,tblclients.groupid,tblclients.status FROM tblclients" . $tableJoin . $where . " ORDER BY " . $this->getPageObj()->getOrderBy() . " " . $this->getPageObj()->getSortDirection() . " LIMIT " . $this->getQueryLimit();
